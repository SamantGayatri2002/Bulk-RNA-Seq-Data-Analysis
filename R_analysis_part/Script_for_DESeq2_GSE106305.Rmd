---
title: "DESeq2_Analysis_of_GSE106305"
...
getwd() #To check where we are already at
setwd("E:/R_analysis_part") #setting our working directory so that all our results will be inside this folder
```


#installing and loading necessry packages
...
# Utilising the packages to read and manupulate the count matrix generated from previous analysis
# if we set include =False, then also the code runs, but we cant able to see the output
...
#library("DESeq2")
library("tidyverse")#This is a meta package , Installing/loading tidyverse loads several packages at once, including:
...
# For the next code we need annotation file GRCh38annotation.csv 
# Either we can download it manually from Enseml BioMart or through terminal
# here we are downloading Through terminal
...
# Get annotation
annotation <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol", "gene_biotype"),
                    mart = mart)
...
##from the above code a new csv file will be generated in our working directory, that can be used for further analysis.

```{r}
#install.packages("data.table")  #when the data is too large, intead of read.csv(), we can use data,table, to use the function fread

library(dplyr) # To use the function select, but when using in code specify as dplyr::select, because multiple packages have select function, so if we not specify this it will throw error
...
png(filename = "variable_gene_heatmap.png", 
    width = 1200, height = 1000, res = 300)  # adjust width/height as needed
...
png("E:/R_analysis_part/de_gene_heatmap.png",
    width = 800, height = 800, res = 150)  # Adjust dimensions and resolution
...
png("E:/R_analysis_part/density_plots_raw_vst.png",
...
# for pathway enrichment with default parameters
enp_gsea <- gsePathway(
  ngenes,
  organism = "human",
  #pvalueCutoff = 0.05,
...
#For pathway enrichment with customised parameters for more precise results
...
```
```{r}
summary(lncap_ranked_list)
any(!is.finite(lncap_ranked_list))
...
# Example: plot for the "HALLMARK_GLYCOLYSIS" pathway
png("fgsea_plot.png", width = 800, height = 600, res = 150)

plotEnrichment(
  hallmark_pathway[["HALLMARK_GLYCOLYSIS"]],
  lncap_ranked_list
) + ggtitle("HALLMARK_GLYCOLYSIS")
...
```{r}
png("fgsea_waterfall.png", width = 1400, height = 1000, res = 150)  # wider + taller
waterfall_plot(fgsea_results, "Hallmark pathways altered by hypoxia in LNCaP cells")
dev.off()
```
